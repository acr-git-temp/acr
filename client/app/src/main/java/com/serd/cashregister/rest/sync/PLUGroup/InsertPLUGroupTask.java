...
public class InsertPLUGroupTask extends ISynchronizerTask implements InsertTemplate.Listener<PLUGroup> {
    public static final String TAG = "InsertPLUGroupTask";
    ...
    public static void prepare(Synchronizer pSynchronizer){
        ...
        if (c.moveToFirst())
        {
            do
            {
                ...
                pSynchronizer.addTask(new InsertPLUGroupTask(pSynchronizer, grp));
            }
            while (c.moveToNext());
        }
        c.close();
    ...
    public InsertTemplate<PLUGroup> mTaskObject;
    public void execute() {mTaskObject.execute();}

    public void onInsertResponse(PLUGroup pluGroup)
    {
