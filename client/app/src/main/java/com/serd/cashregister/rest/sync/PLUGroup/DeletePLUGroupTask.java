...
public class DeletePLUGroupTask extends ISynchronizerTask implements DeleteTemplate.Listener<PLUGroup> {
    public static final String TAG = "DeletePLUGroupTask";
    ...
    public static void prepare(Synchronizer pSynchronizer){
        ...
        if (c.moveToFirst())
        {
            do
            {
                ...
                PLUGroup grp = new PLUGroup(c.getString(1), c.getString(2), c.getString(3), c.getString(5).replace(' ', 'T'),c.getString(5));
                pSynchronizer.addTask(new DeletePLUGroupTask(pSynchronizer, grp, grp.getId()));
            }
            while (c.moveToNext());
        }
        c.close();
    ...
    public DeleteTemplate<PLUGroup> mTaskObject;
    public void execute() {mTaskObject.execute();}

    public void onDeleteResponse(PLUGroup pluGroup)
    {
